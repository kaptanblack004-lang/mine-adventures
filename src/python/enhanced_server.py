...
async def cleanup_connection(websocket):
    ...
    if room_id and room_id in connected_clients:
        ...
        if player_info:
            logging.info(f"Oyuncu {player_info.get('name', 'Unknown')} odadan ayrıldı: {room_id}")
...
async def start_websocket_server(port=8766):
    """WebSocket sunucusunu başlat"""
    async with websockets.serve(websocket_handler, "0.0.0.0", port):
        logging.info(f"WebSocket sunucusu başlatıldı: ws://0.0.0.0:{port}")
