...
logging.basicConfig(
    level=logging.INFO,  # Sadece uyarılar ve hatalar
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('server.log'),
        logging.StreamHandler()  # Konsol logunu ekle - debug için
    ]
)
...
def web_sunucusu_baslat(host='localhost', port=8000, ssl_enabled=False, config=None):
    ...
    try:
        ...
        import threading
        ws_thread = threading.Thread(target=lambda: asyncio.run(start_websocket_server_async(8765)))
